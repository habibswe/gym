<?php

?>

<!DOCTYPE html>
<html>
<head>
	<title>MyGym | Members</title>
	<link rel="stylesheet" type="text/css" href="styles/style.css" media="all" />
	<link rel="stylesheet" type="text/css" href="styles/members_button.css" media="all" />
</head>
<body>
	<div class="wrapper">

		<div class="header">
			<a href="#"><img src="images/header.jpg"></a>
		</div>

		<div class="content_wrapper">
			<div class="right" style="background-image: url(images/right.jpg)">
				<div class="btn_hp">
					<br><br>

					<a href="home.php"><button>Home</button></a>
					<a href="members_button.php"><button>Members</button></a>
					<a href="trainers_button.php"><button>Trainers</button></a>
					<a href="exercises_button.php"><button>Exercises</button></a>
					<a href="admins_button.php"><button>Admins</button></a>
					<a href="index.php"><button>Logout</button></a>
					
				</div>
			</div>
			<div class="left" style="background-image: url(images/bg1.jpg)">
				<div class="middle">

					<br>

					<center><label style="color: white;"><h1>Gym and Fitness | Members</h1></label></center>

					<br><br><br>

					<a href="view_all_members.php"><button>View all Members</button></a>
					<a href="search_member.php"><button>Search Member</button></a>
					<a href="add_member.php"><button>Add Member</button></a>
					<a href="update_member_info.php"><button>Update Member's Information</button></a>
					<a href="delete_member.php"><button>Delete Member</button></a>
					<a href="home.php"><button>Home Page</button></a>	
				</div>
			</div>
		</div>

		<div class="footer">
			<h5 style="color:#000; padding-top:30px; text-align:center; font-family: Verdana, Geneva, sans-serif">&copy; 2022 all rights reserved | Developed By: <a href="#">HABIB</a></h5>
		</div>

	</div>

</body>
</html>